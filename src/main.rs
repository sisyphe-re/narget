...
use libnar::{de::{EntryKind, Entries}, Archive};
...
fn extract_hash(input: &str) -> Option<&str> {
    ...
    RE.captures(input).and_then(|cap| {
        cap.name("hash").map(|hash| hash.as_str())
    })
}
    ...
    for entry in entries {
        ...
        if entry.is_dir() {
            ...
            create_dir(&dst.join(entry.name()));
                ...
                println!("Extracting file {:?} to {:?}", entry.name(), dst.join(entry.name()));
                entry.unpack_in(dst)?
...
fn getArchiveUrl(hash: &str) -> Result<String, MyError>{
    let binaryCaches = ["https://cache.nixos.org/", "https://sisyphe.cachix.org/", "https://bincache.grunblatt.org/"];
    for cache in binaryCaches.iter() {
        let url = format!("{}{}.narinfo", cache.to_owned(), hash);
        let resp = reqwest::blocking::get(url)?;
        match resp.status() {
            reqwest::StatusCode::OK => {
                return Ok(cache.to_string() + &resp.text()?.split("\n").collect::<Vec<&str>>()[1].to_string().split(": ").collect::<Vec<&str>>()[1].to_string());
            },
            _ => {}
        }
    }
...
fn download(path_or_hash: &str) -> Result<(), MyError> {
    ...
    match resp.status() {
        reqwest::StatusCode::OK => {
            ...
            let mut decompressor = XzDecoder::new(resp);
...
fn main() -> Result<(), MyError> {
    let hash = env::args().nth(1).expect("Expected path name or hash of a nar archive");
    download(&hash);
    Ok(())
}